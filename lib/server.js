...
Server.prototype.serve = function(file, index) {
  var ext, match, permised = ['css', 'js', 'html'], controller;
  ...
  if (this.router !== null) {
    return this.router.get(index, controller);
  }
...
Server.prototype.listen = function(callback) {
  ...
  this.io.sockets.on('connection', function(socket) {
    socket.on('ready', function() {
      var context = new Context(socket, self);
      context.router = self.router; // For adding files, later
